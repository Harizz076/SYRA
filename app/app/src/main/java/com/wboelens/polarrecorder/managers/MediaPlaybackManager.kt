...
class MediaPlaybackManager(
    ...
    private fun handleMetadataChange(controller: MediaController, metadata: MediaMetadata) {
        ...
        // Use getText() instead of getString() to safely handle SpannableString metadata
        // on OEM ROMs (ColorOS/OxygenOS) that wrap values in SpannableString — getString()
        // would throw ClassCastException on those devices.
        val newTitle = metadata.getText(MediaMetadata.METADATA_KEY_TITLE)?.toString()
        val newArtist = metadata.getText(MediaMetadata.METADATA_KEY_ARTIST)?.toString()
        val newAlbum = metadata.getText(MediaMetadata.METADATA_KEY_ALBUM)?.toString()
    ...
    private fun updateCurrentMetadata(metadata: MediaMetadata) {
        // Safe metadata extraction: getText() handles SpannableString values on OEM devices.
        currentTrackTitle = metadata.getText(MediaMetadata.METADATA_KEY_TITLE)?.toString()
        currentArtist = metadata.getText(MediaMetadata.METADATA_KEY_ARTIST)?.toString()
        currentAlbum = metadata.getText(MediaMetadata.METADATA_KEY_ALBUM)?.toString()
