...
class FileSystemDataSaver(
  ...
  private fun checkAndRotateFile(deviceId: String, dataType: String) {
    ...
    // ponytail: never rotate LOG files — _part2.jsonl breaks downstream analysis (BUG-09)
    if (dataType == "LOG") return
  ...
  override fun saveData(
    ...
    synchronized(lock) {
      try {
        ...
        // ponytail: stream not ready yet — retry once after 200ms instead of dropping
        scope.launch {
          kotlinx.coroutines.delay(200)
          val retryKey = "$deviceId/$dataType"
          val retryStream = outputStreams[retryKey]
          if (retryStream != null) {
            try {
              retryStream.second.write((payload + "\n").toByteArray())
            } catch (retryEx: Exception) {
              logViewModel.addLogError("[$deviceId] Retry failed for $dataType: ${retryEx.message}")
            }
          } else {
            logViewModel.addLogError("[$deviceId] Stream still not ready after retry, dropping: $dataType")
          }
        }
        Unit
      }
