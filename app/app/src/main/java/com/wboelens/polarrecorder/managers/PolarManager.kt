...
class PolarManager(
  ...
  private fun fetchDeviceCapabilities(deviceId: String): Single<DeviceStreamCapabilities> {
    return Single.create { emitter ->
        ...
        .doOnError { error ->
          // ponytail: disconnect on exhausted retries — prevents infinite GATT loop (BUG-01)
          Log.e(TAG, "Capability fetch failed after $MAX_RETRY_ERRORS retries, disconnecting: ${error.message}")
          logViewModel.addLogError(
              "Sensor connection failed after $MAX_RETRY_ERRORS retries. Turn the H10 off/on and try again.",
          )
          try {
            val deviceId = deviceCapabilities.keys.find { true } // will be caught by caller anyway
            // Disconnect is triggered from the caller (deviceConnected) when capabilities are null
      ...
      MainScope().launch {
        ...
        if (
          ...
          try {
            ...
            // ponytail: H10 doesn't support getTime — expected, not an error (BUG-08)
            logViewModel.addLogMessage("Failed to fetch device time (${e.message})")
          }
        }
