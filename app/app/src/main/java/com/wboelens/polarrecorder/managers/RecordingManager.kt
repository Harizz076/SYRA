...
import com.wboelens.polarrecorder.services.PolarSensorService
...
class RecordingManager(
  ...
  fun startRecording() {
    ...
    // Start the foreground sensor service only in PHYSIOLOGY mode — other modes
    // do not need a WakeLock or BLE data streams, and starting the service without
    // a sensor would cause a SecurityException crash on Android 14+.
    if (preferencesManager.requiresSensor) {
        val serviceIntent = Intent(context, com.wboelens.polarrecorder.services.PolarSensorService::class.java).apply {
            action = com.wboelens.polarrecorder.services.PolarSensorService.ACTION_START
        }
  ...
  fun stopRecording() {
    ...
    Handler(Looper.getMainLooper()).post {
      ...
      // Stop the foreground service (only running when requiresSensor is true)
      if (preferencesManager.requiresSensor) {
          val stopIntent = Intent(context, com.wboelens.polarrecorder.services.PolarSensorService::class.java).apply {
              action = com.wboelens.polarrecorder.services.PolarSensorService.ACTION_STOP
          }
  ...
  fun stopMusicPostRecording() {
    // ponytail: idempotent — if already stopped or flags are wrong, just clean up (BUG-03)
    if (!isMusicPostRecording && !_isRecording.value) {
      cancelMusicPostTimeout()
      return
    }
